...
module.exports = {
    data: {
        id: 'user-select-economy-inventory',
        description: 'When the member provides the user who\'s inventory it would like to change'
    },
    run: function(client, interaction){
        ...
        new Promise(async (resolve, reject) => {
            ...
            .setDescription(`Please confirm that you want to change <@!${member.id}>\'s inventory`)
            .setTimestamp();

            const confirmBtn = new ButtonBuilder()
            .setStyle(ButtonStyle.Success)
            .setLabel(`Confirm`)
            .setCustomId(`confirm-economy-inventory-member`);
            const cancelBtn = new ButtonBuilder()
            .setStyle(ButtonStyle.Danger)
            .setLabel(`Cancel`)
            .setCustomId(`cancel-economy-inventory-member`);

            const actionRow = new ActionRowBuilder().addComponents(confirmBtn, cancelBtn);

            client.interactionActions.set(`change-inventory-member-${interaction.member.id}-${interaction.guild.id}`, member);
