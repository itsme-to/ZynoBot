...
module.exports = {
    data: {
        id: 'user-select-xp',
        description: 'The user whose xp should be changed'
    },
    run: function(client, interaction){
        ...
        new Promise(async (resolve, reject) => {
            ...
            .setLabel(`Confirm`)
            .setCustomId(`confirm-xp-member`);
            const cancelBtn = new ButtonBuilder()
            .setStyle(ButtonStyle.Danger)
            .setLabel(`Cancel`)
            .setCustomId(`cancel-xp-member`);

            const actionRow = new ActionRowBuilder().addComponents(confirmBtn, cancelBtn);

            client.interactionActions.set(`xp-member-${interaction.member.id}-${interaction.guild.id}`, member);
