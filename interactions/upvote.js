const { EmbedBuilder, ButtonBuilder, ButtonStyle, ActionRowBuilder, PermissionsBitField, TextInputStyle, ModalBuilder, TextInputBuilder } = require("discord.js");
...
module.exports = {
    data: {
        id: 'upvote-suggestion',
        description: 'When a suggestion gets upvoted',
    ...
    run: async function(client, interaction){
        ...
        if(interaction.member.permissions.has(PermissionsBitField.Flags[data.permissions]) && interaction.member.roles.cache.filter(r => client.config.moderator_roles[interaction.guild.id].indexOf(r.id) >= 0).size > 0 && remove === false){
            const approveAction = new EmbedBuilder()
            .setColor(client.embedColor)
            .setAuthor({name: client.user.username, iconURL: client.user.displayAvatarURL({dynamic: true})})
            .setTitle(handleMessage(client, interaction.member.user, undefined, interaction.channel, messages.general.suggest.actions.approve.title))
            .setDescription(handleMessage(client, interaction.member.user, undefined, interaction.channel, messages.general.suggest.actions.approve.message))
            .setTimestamp();

            const confirmBtn = new ButtonBuilder()
            .setStyle(ButtonStyle.Success)
            .setLabel(`Yes`)
            .setCustomId(`confirm-approve`);
            const cancelBtn = new ButtonBuilder()
            .setStyle(ButtonStyle.Danger)
            .setLabel(`No`)
            .setCustomId(`cancel-approve`);

            const actionRow = new ActionRowBuilder().addComponents(confirmBtn, cancelBtn);

            interaction.reply({embeds: [approveAction], components: [actionRow], ephemeral: true, fetchReply: true}).then(async msg => {
                client.interactionInfo.get(`ignore`).set(msg.id, true);
                const filter = i => i.user.id === interaction.member.user.id && ['confirm-approve', 'cancel-approve'].includes(i.customId);
                const collector = msg.createMessageComponentCollector({filter: filter, time: 2*60*1000, max: 1});

                collector.on('collect', async i => {
                    if(i.customId === 'confirm-approve'){
                        client.interactionActions.set(`suggestion-${interaction.member.id}-${interaction.guild.id}`, {approve: true, messageId: interaction.message.id, userId: suggestionInfo.userId, suggestion: suggestionInfo.suggestion, upvotes: suggestionInfo.upvotes, downvotes: suggestionInfo.downvotes});
                        ...
                        i.update({embeds: [cancelledEmbed], components: []}).catch(err => {});
                    }
                });
            ...
            .setColor(client.embedColor)
            .setAuthor({name: client.user.username, iconURL: client.user.displayAvatarURL({dynamic: true})})
            .setTitle(handleMessage(client, interaction.member.user, undefined, interaction.channel, messages.general.suggest.confirmation.upvote.title))
            .setDescription(handleMessage(client, interaction.member.user, undefined, interaction.channel, messages.general.suggest.confirmation.upvote.message, [{ACTION: action}]))
            .setTimestamp();
            
            interaction.reply({embeds: [confirm], ephemeral: true, fetchReply: true}).catch(err => {});
        }
    }
}