...
module.exports = {
    ...
    run: function(client, args, message, interaction){
        ...
        .setAuthor({iconURL: client.user.displayAvatarURL({dynamic: true}), name: client.user.username})
        .setTitle(handleMessage(client, message.member.user, undefined, message.guild.members.me.voice.channel, messages['music-cmds']['loop-embeds'].loop.title))
        .setDescription(handleMessage(client, message.member.user, undefined, message.guild.members.me.voice.channel, messages['music-cmds']['loop-embeds'].loop.message))
        .setTimestamp();

        client.audioManager.loop(message.guild.members.me.voice.channel, client.audioManager.looptypes.loop);
